"""
Nova Desktop Control Agent — frozen entrypoint.

This is the script PyInstaller freezes into `Nova-agent.exe`. It runs the
...
Run (frozen):   Nova-agent.exe
Run (dev):      python run_agent.py
Environment:
    Nova_AGENT_HOST   default 127.0.0.1
    Nova_AGENT_PORT   default 8765
    Nova_DATA_DIR     where logs/ is written (default: cwd)
"""
...
def _resolve_data_dir() -> Path:
    data = os.environ.get("Nova_DATA_DIR") or os.getcwd()
    logs = Path(data) / "logs"
    try:
...
def main() -> None:
    ...
    log = logging.getLogger("Nova.agent.boot")
    ...
    log.info("Starting Nova agent (frozen=%s) on %s:%d", frozen, host, port)
