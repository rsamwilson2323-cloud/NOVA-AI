"""
Windows auto-start management for NOVA (V2).

Manages a single registry entry under
    HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\Nova
which points at the silent launcher batch file (start-nova-silent.bat) located
...
VALUE_NAME = "Nova"
SILENT_LAUNCHER = "start-nova-silent.bat"
...
def _ensure_launcher_exists() -> str:
    """
    Make sure start-nova-silent.bat exists in the project root.
...
def enable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start enabled. Nova will launch silently on next Windows login.",
        "enabled": True,
        "launcher": launcher,
...
def disable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start disabled. Nova will no longer launch on login.",
        "enabled": False,
    }
...
def get_auto_start_status(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": (
            "Auto-start is ENABLED. Nova launches on Windows login."
            if enabled
